%% GNG function
function net = GrowingNeuralGasNetworkJoint(inputData, params, plotFlag,objSelect)%,objSelect

% % alphaDist = 0.85;
% % alphaParam = alphaDist/2;
% % betaDist = 1 - alphaDist;
% % betaParam = betaDist/6;
alphaParam = 1;
betaParam = 1;
% % wVec = [betaParam betaParam alphaParam alphaParam betaParam betaParam betaParam betaParam];
if objSelect == 1
    wVec = [betaParam betaParam alphaParam alphaParam betaParam betaParam betaParam betaParam];
elseif objSelect == 2
    wVec = [betaParam betaParam betaParam betaParam betaParam betaParam alphaParam alphaParam];
else
    error('Please put obj = 1 or obj = 2')
end
%% GNG parameters
...
%         timeLocal = [timeLocal; toc(startLocal)];
    ...
    if plotFlag
        ...
        figure(1)
        if objSelect == 1
            inputNormObj = [inputNorm(:,1) inputNorm(:,2)];
            wNormObj = [wNorm(:,1) wNorm(:,2)];
        elseif objSelect == 2
            inputNormObj = [inputNorm(:,5) inputNorm(:,6)];
            wNormObj = [wNorm(:,5) wNorm(:,6)];
        else
            error('Please put obj = 1 or obj = 2')
        end
...
for i = 1:size(datanodes,2)
    ...
    if objSelect == 1
        nodesRadAccept(1,i) = sqrt(sum((3*std(datanodes{1,i}(:,1:2))).^2));
        nodesRadAcceptNorm(1,i) =sqrt(sum((3*std(datanodesNorm{1,i}(:,1:2))).^2));
    elseif objSelect == 2
        nodesRadAccept(1,i) = sqrt(sum((3*std(datanodes{1,i}(:,5:6))).^2));
        nodesRadAcceptNorm(1,i) =sqrt(sum((3*std(datanodesNorm{1,i}(:,5:6))).^2));
    end
end
