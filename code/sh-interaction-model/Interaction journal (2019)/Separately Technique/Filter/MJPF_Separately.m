function estimationAbn = MJPF_Separately(odometric, control,...
...
radiusOdm = [odometric.radiusStateOdm;odometric.radiusDivOdm];
radiusCont = [control.radiusStateCon;control.radiusDivCon];
% Mean and covariance
UOdm = [odometric.averageStateOdm,odometric.averageDivOdm];
UCont = [control.averageStateCon,control.averageDivCon];
...
Pinit = eye(numState*2)*r2; %eye(4)*r1;
...
for i = 1:DatasetSize
    ...
    for n = 1:N
        ...
        [probDistOdm,~] =...
            nodesProb_New(currStateOdm(1:2,1)',UOdm(:,1:2),radiusOdm(1,:));  %   sum(radiusOdm(1:2,:))/2   %   Find probability of being in each superstate of voc1
        [probDistCont,~] =...
            nodesProb_New(currStateCont(1:2,1)',UCont(:,1:2),radiusCont(1,:)); %sum(radiusCont(1:2,:))/2
        ...
        if i > firstIt
            ...
            d1Odm_Probabilistic(n,i-1) = max([(bhattacharyyadistance(statepredOdm(:,i-1,n)',...
                ...
                positivedefinite(covarianceOdm(:,:,nodeIndOdm))) - minOdmAbn1)/normOdmAbn1,0]);
            d1Cont_Probabilistic(n,i-1)  = max([(bhattacharyyadistance(statepredCont(:,i-1,n)',...
                ...
                positivedefinite(covarianceCont(:,:,nodeIndCont)))- minContAbn1)/normContAbn1,0]);
            ...
            if n == N
                ...
                for ij = 1:N
                    ...
                    meanCountOutside(i,1) = countOutside/N;
